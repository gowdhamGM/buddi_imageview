...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def playSlideShow(self, checked=True):
        # print(checked)
...
def image_module(message):
    win=Window()
    
    if "next image" in message:
        win.openNextImage()
    elif "previews image" in message:
        win.openPrevImage()
    elif "zoom in" in message:
        win.zoomInImage()
    elif "zoom out" in message:
        win.zoomOutImage()
    elif "rotate right" in message:
        win.rotateRight()
        ...
        if os.path.exists(path):
            ...
            time.sleep(1)
            win.playSlideShow()
    elif "slide show stop" in message:
        win.playSlideShow()
...
def main():
    ...
    app.setOrganizationName("ImageView")
